...
# Add the web-ui directory to the Python path for imports
sys.path.insert(0, str(Path(__file__).parent.parent / "web-ui"))

def test_streamlit_app_imports():
    """Test that the Streamlit app can be imported without errors."""
    try:
        import main  # This should now work with web-ui in the path
        assert True, "Streamlit app imported successfully"
    except ImportError as e:
        pytest.fail(f"Failed to import main as streamlit_app: {e}")

@pytest.mark.xfail(reason="UI function testing requires streamlit context")
...
def test_validation_display_function():
    ...
    import main as streamlit_app
    from csv_validator import ValidationResult
